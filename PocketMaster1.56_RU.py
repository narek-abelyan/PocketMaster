            ...
            if not os.path.exists(dest_path):
                ...
                if r.status_code == 200:
                    with open(dest_path, 'w') as f:
                        f.write(r.text)
                    print(f"⬇️  Загружено: {pdb_id} в {output_folder}")
...
if args.config:
    ...
    if not config:
        print("Не удалось загрузить конфигурацию (config). Переход в интерактивный режим.")
        config = {}
else:
    config = {}
...
if not do_preprocess:
    ...
    if do_preprocess == '1':
        ...
        print("2 — Удалить ионы  (Mg²⁺, Cl⁻, и др.)")
...
if not pocket_method:
    ...
    print("2 – После предварительного выравнивания всех структур, для каждой структуры вокруг выбранного реф. остатка")
...
if not rmsd_method_input:
    ...
    rmsd_method_input = input("\nВведи метод (1, 2, 3, 4 или 5): ").strip()
...
if not cl_choice:
    print("\nВыберите параметры кластеризации:")

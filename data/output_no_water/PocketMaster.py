...
if comparison_mode == "1":
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            ...
            rms_ca, reason_ca = compute_rmsd(sel1_ca, sel2_ca)
...
with open("info.txt", "w", encoding="utf-8-sig") as info_file:
    ...
    if pocket_method == "1":
        info_file.write(f"Метод определения кармана: Радиальное (радиус {radius} Å от остатка {ligand_resi})\n")
    elif pocket_method == "2":
...
if plt and sns:
    ...
    plt.axvline(np.mean(values), color='red', linestyle='--', label='Среднее')
    plt.legend()

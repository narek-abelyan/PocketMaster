...
if method_choice in method_map:
    linkage_method = method_map[method_choice]
    print(f"Выбран метод кластеризации: {linkage_method}")
else:
    linkage_method = "ward"  # метод по умолчанию
    print(f"Неверный ввод, выбран метод по умолчанию: {linkage_method}")
...
cl_choice = input("Ваш выбор (1, 2 или 3): ").strip()


if cl_choice == '1':
    try:
        num_clusters = int(input("Введите желаемое количество кластеров: ").strip())
    except ValueError:
        print("Некорректное число кластеров.")
elif cl_choice == '2':
    try:
        distance_threshold = float(input("Введите порог расстояния (например, 5.0): ").strip())
    except ValueError:

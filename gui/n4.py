...
def visualize_structure_interactive(filepath):
    ...
    style = st.selectbox("Стиль визуализации", ["stick", "line", "surface", "sphere", "cartoon", "cross"], index=0)
    ...
    if style == "surface":
        viewer.setStyle({"surface": {"opacity": opacity, "color": color}})
    else:
...
with tabs[0]:
    ...
    if mode.startswith("1"):
        ...
        if p.exists() and p.is_dir():
            ...
            if mol_files:
                ...
                st.info("В папке нет подходящих файлов структур.")
        else:
            st.warning("Папка не найдена или недоступна.")
    ...
    ligand_resi = st.text_input("Номер остатка (метод 1)", value="40")
    ligand_chain = st.text_input("Chain ID (метод 1)", value="A")
